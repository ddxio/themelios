...
install_arguments="--no-root-passwd"        # any extra arguments to nixos-install, like --no-root-passwd
zfs_pool_name="tank"
zfs_pool_disks=("/dev/sdc") # Note: using /dev/disk/by-id is also preferable.
zfs_pool_cache_disks=("/dev/nvme0n1")
zfs_pool_log_disks=("/dev/sdb")
